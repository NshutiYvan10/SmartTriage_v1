...
export function IoTDeviceManagement() {
  ...
  // Theme-aware action-pill palettes. The dark values (translucent /20 fills
  // + 300-weight text) are near-invisible on the light theme's white cards, so
  // light mode uses solid pastel fills with 700/800 text for readable contrast.
  // Full literal strings — Tailwind cannot see dynamically-built class names.
  const pill = {
    emerald: isDark
    ...
    cyan: isDark
      ...
      : 'bg-cyan-100 text-cyan-800 border border-cyan-400 hover:bg-cyan-200',
    slate: isDark
      ? 'bg-slate-500/20 text-slate-300 border border-slate-500/30'
      : 'bg-slate-100 text-slate-700 border border-slate-300',
  };
  // Icon tints paired with the pills above (icons need the same light/dark split).
  const pillIcon = {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 mt-3 pt-3" style={{ borderTop: isDark ? '1px solid rgba(2,132,199,0.12)' : '1px solid rgba(203,213,225,0.2)' }}>
                    ...
                    {isAdmin && (
                      <div className={`flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg ${device.inService ? pill.emerald : pill.slate}`}>
                        <div className="flex items-center gap-1.5">
                          {device.inService
                            ? <CheckCircle className={`w-3.5 h-3.5 ${pillIcon.emerald}`} />
                            : <PowerOff className={`w-3.5 h-3.5 ${pillIcon.slate}`} />}
                          <span className={`text-[10px] font-bold ${device.inService ? (isDark ? 'text-emerald-300' : 'text-emerald-800') : text.muted}`}>
                    ...
                    {isAdmin && !device.inService && (
                      <button
                        ...
                        className={`flex-1 px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.emerald} transition-colors flex items-center justify-center gap-1 disabled:opacity-50`}
                      >
                    ...
                    {isAdmin && device.inService && (
                      <button
                        ...
                        disabled={serviceLoadingId === device.id}
                        className={`flex-1 px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.red} transition-colors flex items-center justify-center gap-1 disabled:opacity-50`}
                      >
                      ...
                      <button
                        ...
                        className={`flex-1 px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors flex items-center justify-center gap-1 disabled:opacity-50 ${
                          device.triageMonitor
                            ? pill.purple
                            : `${text.body} border ${isDark ? 'border-slate-500/30 hover:bg-white/5' : 'border-slate-300 hover:bg-slate-100'}`
                        }`}
                      >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className={`flex-1 px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.amber} transition-colors flex items-center justify-center gap-1 disabled:opacity-50`}
                      >
                    ...
                    {isClinicalStaff && device.status === 'ONLINE' && (
                      <button onClick={() => handleStartMonitoring(device)} className={`flex-1 px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.cyan} transition-colors flex items-center justify-center gap-1`}>
                    ...
                    {(() => {
                      ...
                      if (attachedBed) {
                        return (
                          <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg ${pill.emerald}`}>
                            <BedDouble className={`w-3.5 h-3.5 ${pillIcon.emerald} flex-shrink-0`} />
                            <div className="flex-1 min-w-0">
                              <p className={`text-[10px] font-bold ${isDark ? 'text-emerald-300' : 'text-emerald-800'}`}>
                                Bed {attachedBed.code} · {attachedBed.zone}
                              </p>
                              {attachedBed.label && (
                                <p className={`text-[9px] truncate ${isDark ? 'text-emerald-300/80' : 'text-emerald-700'}`}>{attachedBed.label}</p>
                              )}
                            </div>
                            {(isAdmin || isClinicalStaff) && (
                              <button
                                ...
                                className={`text-[9px] font-bold underline-offset-2 hover:underline disabled:opacity-50 ${isDark ? 'text-emerald-400 hover:text-emerald-300' : 'text-emerald-800'}`}
                      ...
                      return (isAdmin || isClinicalStaff) && device.status !== 'DECOMMISSIONED' ? (
                        <button
                          ...
                          className={`flex-1 px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.emerald} transition-colors flex items-center justify-center gap-1`}
                        >
                    ...
                    {isClinicalStaff && (device.status === 'REGISTERED' || device.status === 'OFFLINE') && (
                      <div className={`flex items-center gap-1.5 px-2.5 py-2 rounded-lg ${isDark ? 'bg-amber-500/20 border border-amber-500/30' : 'bg-amber-100 border border-amber-400'}`}>
                        <AlertTriangle className={`w-3.5 h-3.5 flex-shrink-0 ${isDark ? 'text-amber-400' : 'text-amber-700'}`} />
                        <p className={`text-[10px] font-medium ${isDark ? 'text-amber-300' : 'text-amber-900'}`}>
                    ...
                    {device.status === 'MONITORING' && (() => {
                      ...
                      return (
                        <div className="flex-1 space-y-1.5">
                          {pt && (
                            <div className={`flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-lg ${isDark ? 'text-cyan-300 bg-cyan-500/20' : 'text-cyan-800 bg-cyan-100 border border-cyan-300'}`}>
                          ...
                          {isClinicalStaff && session && (
                            <button onClick={() => handleStopMonitoring(session.id)} className={`w-full px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.red} transition-colors flex items-center justify-center gap-1`}>
                          ...
                          {isAdmin && device.inService && (
                            <button
                              onClick={() => { setServiceError(null); setOutOfServiceTarget(device); }}
                              disabled={serviceLoadingId === device.id}
                              className={`w-full px-3 py-1.5 text-[10px] font-bold rounded-lg ${pill.red} transition-colors flex items-center justify-center gap-1 disabled:opacity-50`}
                            >
                              {serviceLoadingId === device.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <PowerOff className="w-3 h-3" />} Take Out of Service
                            </button>
                          )}
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 text-[10px] font-bold rounded-lg transition-colors border ${
                      sessionZoneFilter === null
                        ? (isDark ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' : 'bg-cyan-100 text-cyan-800 border-cyan-400')
                        : `${text.body} border-transparent ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-100'}`
                    }`}
                  >
                    All ({sessions.length})
                  </button>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-extrabold uppercase tracking-wider border ${
                                b.status === 'AVAILABLE' ? (isDark ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : 'bg-emerald-100 text-emerald-800 border-emerald-400')
                                  : b.status === 'OCCUPIED' ? (isDark ? 'bg-slate-500/20 text-slate-300 border-slate-500/30' : 'bg-slate-100 text-slate-700 border-slate-300')
                                  : b.status === 'CLEANING' ? (isDark ? 'bg-amber-500/20 text-amber-300 border-amber-500/30' : 'bg-amber-100 text-amber-900 border-amber-400')
                                  : (isDark ? 'bg-rose-500/20 text-rose-300 border-rose-500/30' : 'bg-rose-100 text-rose-800 border-rose-400')
                              }`}>
                                {b.status}
                              </span>
                            </div>
            ...
            <div className="w-full max-w-md rounded-2xl p-6 overflow-hidden shadow-2xl animate-scale-in" style={glassCard}>
              ...
              <div className="space-y-3 mb-5">
                ...
                <div>
                  ...
                  <div className={`flex items-center gap-2 p-3 rounded-xl font-mono text-xs break-all ${isDark ? 'text-emerald-300' : 'text-emerald-800'}`} style={glassInner}>
              ...
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl mb-4 ${isDark ? 'bg-amber-500/20 border border-amber-500/30' : 'bg-amber-100 border border-amber-400'}`}>
                <AlertTriangle className={`w-4 h-4 flex-shrink-0 ${isDark ? 'text-amber-400' : 'text-amber-700'}`} />
                <p className={`text-[10px] ${isDark ? 'text-amber-300' : 'text-amber-900'}`}>
