...
export function AdultTriageForm() {
  ...
  //
  // patientStore.fetchActiveVisits hardcodes `age: 0` because the visit
  // list payload is name-only — the same data path that produced the
  // "0mo · M" rendering on Luna Gisa in the triage queue. On form open
  // we fetch the full patient and overwrite the empty initial state for
  // dob / patientAge / gender / names (when blank) so the nurse sees
  // the same identity fields she captured at registration without
  // having to re-type them. Read-only behaviour is mirrored from
  // EntryRegistration: when DOB is present, age is auto-derived and
  ...
  useEffect(() => {
    ...
    patientApi.getById(backingPatientId)
      .then((p) => {
        ...
        // Fill identity fields when the list payload didn't carry them.
    ...
    // patient.patientId is the backing-row id (distinct from visit id). We
    // intentionally do NOT depend on `patient` itself — that would loop on
    // every store update. Re-fire only when the underlying patient row
    // changes (effectively never within a single triage form lifetime).
  ...
  // Footer
  // Triage nurse is the logged-in user — captured automatically, never typed.
  // Synced via useEffect below so a delayed authUser hydration still
  // populates the field before submit-validation runs.
      ...
      <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-4 animate-fade-in">
        ...
        {showDeteriorationWarning && (
          <div className="bg-amber-500 text-white px-4 py-2.5 flex items-center justify-between rounded-2xl">
            ...
            <button onClick={() => setShowDeteriorationWarning(false)} className="text-[10px] bg-white/20 px-2.5 py-1 rounded-xl hover:bg-white/30">Dismiss</button>
          </div>
        )}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  {/* Age is auto-derived from DOB when DOB is present —
                      same pattern as EntryRegistration. Editable only
                      when DOB is empty (e.g. nurse entered age, never
                      captured a precise DOB). */}
                  <input
                    type="number"
                    value={patientAge}
        ...
        <div className="rounded-2xl overflow-hidden" style={glassCard}>
          ...
          <div className="p-4">
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-200/40">
              <button onClick={() => setSignPage((p) => Math.max(0, p - 1))} disabled={signPage === 0} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-semibold transition-all disabled:opacity-40 disabled:cursor-not-allowed bg-white/80 border border-slate-200/60 text-slate-600 hover:bg-white hover:shadow-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              {/* Triage Nurse is the logged-in user, captured automatically.
                  ...
                  audit-tampering surface — the nurse can't accidentally (or
                  intentionally) type someone else's name into the record. */}
              <div>
