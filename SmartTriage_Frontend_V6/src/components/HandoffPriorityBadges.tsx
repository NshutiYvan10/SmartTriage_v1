...
/**
 * Priority badges rendered on patient cards (Doctor Workspace, Monitoring,
 * any other active-visit list) so an inheriting clinician sees, at a
 * glance, exactly which patients have outstanding work that crossed the
 * shift boundary.
 *
 ...
 */
export function HandoffPriorityBadges({ visit }: { visit: VisitResponse }) {
  const labs = visit.pendingInvestigationsCount ?? 0;
  const critResults = visit.unacknowledgedCriticalResultsCount ?? 0;
  const meds = visit.pendingMedicationsCount ?? 0;
  const icu = !!visit.hasOpenIcuEscalation;

  if (labs === 0 && critResults === 0 && meds === 0 && !icu) {
    return null;
  }
