...
public class ContinuousMonitoringEngine {
    ...
    public MonitoringResult analyseAndRespond(UUID visitId, DeviceSession detachedSession) {
        ...
        if (!alreadyOpen) {
            raiseAndFanOutDeterioration(visit, detectedPattern, description);
            alertCount++;
    ...
    public void evaluateManualVitals(Visit visit, VitalSigns vitals) {
        try {
            ...
            raiseAndFanOutDeterioration(visit, pattern, description);
        } catch (Exception e) {
    ...
    private ClinicalAlert generateDeteriorationAlert(Visit visit,
        ...
        ClinicalAlert alert = ClinicalAlert.builder()
                ...
                .message(String.format(
                        ...
                        pattern.name(),
                        description))
                // Target the patient's current zone so the alert appears in zone-scoped
                // queries / the zone board. Deliberately NO targetDoctor: a deterioration
                // is a monitoring BROADCAST, and when it co-fires with an escalating
                // auto-retriage the accountable-doctor page is the properly-zoned
                // DOCTOR_NOTIFICATION raised by the retriage path (this method runs BEFORE
                // the category is escalated, so a doctor resolved here would be the stale
                // pre-escalation zone's).
                .targetZone(zone)
                .autoGenerated(true)
                .build();
    ...
    /**
     * Raise a DETERIORATION_DETECTED alert and broadcast it to the hospital board + the
     * patient's current zone, AFTER COMMIT (so a rolled-back analysis never pushes a phantom
     * alert). Every doctor receives it on the hospital feed.
     *
     * <p>Deliberately NOT user-targeted to the zone doctor / charge nurse: when a
     * deterioration also triggers an escalating auto-retriage, the auto-retriage path raises
     * a properly-zoned DOCTOR_NOTIFICATION (with tiered re-escalation) to the CORRECT
     * post-escalation zone doctor. User-paging here as well would (a) double-page that doctor
     * and (b) — because this runs before the category is escalated — page the STALE
     * pre-escalation zone's doctor. The hospital+zone broadcast is the right scope for a
     * monitoring signal. Returns the saved alert. Best-effort publish (never breaks detection).
     */
    private ClinicalAlert raiseAndFanOutDeterioration(Visit visit, DeteriorationPattern pattern, String description) {
        UUID hospitalId = visit.getPatient().getHospital().getId();
        EdZone zone = visit.getCurrentTriageCategory() != null
                ? EdZone.fromTriageCategory(visit.getCurrentTriageCategory()) : null;
        ClinicalAlert saved = generateDeteriorationAlert(visit, pattern, description, zone);
        try {
            ClinicalAlertResponse resp = ClinicalAlertMapper.toResponse(saved);
            eventPublisher.publishOwnedAlertAfterCommit(hospitalId, zone, resp, List.of());
        } catch (Exception e) {
