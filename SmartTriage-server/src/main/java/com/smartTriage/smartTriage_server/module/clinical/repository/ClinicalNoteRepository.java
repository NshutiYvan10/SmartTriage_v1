...
public interface ClinicalNoteRepository extends JpaRepository<ClinicalNote, UUID> {
    ...
    /** RBAC fix — projection used by ClinicalAuthz.canAccessClinicalNote. */
    @Query("SELECT n.visit.id FROM ClinicalNote n WHERE n.id = :id")
    Optional<UUID> findVisitIdByNoteId(@Param("id") UUID id);
}
