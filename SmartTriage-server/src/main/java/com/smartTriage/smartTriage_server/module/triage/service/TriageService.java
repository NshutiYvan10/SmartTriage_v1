...
public class TriageService {
    ...
    public TriageRecordResponse confirmFieldTriage(UUID visitId) {
        ...
        // category being attested, not the score, is what confirmation is about. The run is
        // also the route back to the transporting paramedic for the confirmation notice below.
        var emsRun = emsRunRepository.findByVisitIdAndIsActiveTrue(visitId).orElse(null);
        Integer fieldTews = emsRun != null ? emsRun.getFieldTewsScore() : null;
        ...
        // Close the loop back to the transporting paramedic: their field category was
        // ACCEPTED as the ED triage. targetDoctor = the run's paramedic is the delivery
        // key for the PARAMEDIC personal alert scope (AlertScopeResolver) + the
        // /topic/alerts/user/{id} push. Best-effort — never blocks the confirmation.
        try {
            if (emsRun != null && emsRun.getParamedic() != null && visit.getHospital() != null) {
                ClinicalAlert paramedicAlert = ClinicalAlert.builder()
                        .visit(visit)
                        .alertType(AlertType.EMS_FIELD_TRIAGE_CONFIRMED)
                        .severity(AlertSeverity.LOW)
                        .title("Field triage confirmed: " + fieldCategory)
                        .message(String.format(
                                "Your field triage category (%s%s) was accepted as the ED triage by %s.",
                                fieldCategory,
                                fieldTews != null ? ", field TEWS " + fieldTews : "",
                                actorName))
                        .targetDoctor(emsRun.getParamedic())
                        .autoGenerated(true)
                        .build();
                paramedicAlert = clinicalAlertRepository.save(paramedicAlert);
                eventPublisher.publishOwnedAlertAfterCommit(
                        visit.getHospital().getId(), null,
                        ClinicalAlertMapper.toResponse(paramedicAlert),
                        java.util.List.of(emsRun.getParamedic().getId()));
            }
        } catch (Exception e) {
            log.warn("Failed to notify paramedic of confirmed field triage on visit {}: {}",
                    visit.getVisitNumber(), e.getMessage());
        }
    ...
    private void generateHighAcuityAlert(Visit visit, TriageCategory category,
        ...
        // This is the FALLBACK path — reached only when the primary zone-routed alert
        // (createZoneRoutedAlert) threw. It must NOT produce an inert TEWS_CRITICAL that
        // the escalation scheduler ignores: a critical patient whose primary alert failed
        // is exactly the one that must still be re-paged. Raise it as a DOCTOR_NOTIFICATION
        // (the type the tiered re-escalation loop drives — Tier-2 at +2min, Tier-3 +
        // all-staff at +5min) with escalationTier=1 + escalatedAt set, and broadcast it live
        // now (the primary live push did not happen). targetDoctor is intentionally null —
        // resolving it needs the shift lookup that just failed; the scheduler escalates a
        // tier-1 alert regardless of an assigned doctor.
        com.smartTriage.smartTriage_server.common.enums.EdZone zone =
                com.smartTriage.smartTriage_server.common.enums.EdZone.fromTriageCategory(category);
        ...
        // Deliver live, but AFTER COMMIT — the fallback runs inside performTriage's open
        // transaction, which may still roll back on a later step. Pushing synchronously here
        // (as the rest of Batch 2 deliberately stopped doing) would broadcast a phantom
        // CRITICAL triage alert whose row never committed. Route through the shared
        // after-commit helper (hospital + zone broadcast; no user-targeting — the shift
        // lookup is exactly what failed on the primary path).
        ClinicalAlertResponse resp = ClinicalAlertMapper.toResponse(alert);
        UUID hospitalId = visit.getPatient().getHospital().getId();
        eventPublisher.publishOwnedAlertAfterCommit(hospitalId, zone, resp, java.util.List.of());
        log.warn("{} TRIAGE ALERT (fallback DOCTOR_NOTIFICATION): Visit {} — {} (TEWS: {})",
