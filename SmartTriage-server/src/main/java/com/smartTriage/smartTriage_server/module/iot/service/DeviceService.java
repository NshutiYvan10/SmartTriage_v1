...
public class DeviceService {
    ...
    public void handleMonitoringDeviceDisconnect(UUID deviceId) {
        ...
        if (!alreadyOpen) {
            ...
            broadcastDisconnectAlert(visit, alert);
    ...
    /**
     * Push a device-disconnect alert to the hospital + zone alert topics so
     * dashboards update without a refresh. Best-effort — a STOMP failure must
     * not abort the disconnect-handling transaction.
     */
    private void broadcastDisconnectAlert(
        ...
        try {
            ...
            com.smartTriage.smartTriage_server.common.enums.TriageCategory cat =
                    visit.getCurrentTriageCategory();
            if (cat != null) {
                eventPublisher.publishZoneAlert(hospitalId,
                        com.smartTriage.smartTriage_server.common.enums.EdZone.fromTriageCategory(cat),
                        response);
            }
        } catch (Exception e) {
