...
public class DeviceService {
    ...
    /** Lets the heartbeat scheduler raise the device-disconnect alert inside a
     *  transaction — see {@link #handleMonitoringDeviceDisconnect(java.util.UUID)}. */
    private final com.smartTriage.smartTriage_server.module.alert.repository.ClinicalAlertRepository clinicalAlertRepository;
    ...
    /**
     * Handle a monitored device going stale/offline: raise the
     * IOT_DEVICE_DISCONNECTED alert, freeze the last reading into a VitalSigns
     * snapshot, and transition the session to DISCONNECTED — all in ONE
     * transaction.
     *
     * <p>This logic previously lived in {@code DeviceHeartbeatScheduler}, which
     * runs with no open JPA session (open-in-view=false). Reading the lazy
     * {@code session.getVisit().getPatient()} graph there threw
     * {@code LazyInitializationException}, so the alert, the last-reading
     * snapshot, and the DISCONNECTED transition were all silently skipped (the
     * MonitoringStateWatcher masked the alert ~10s later, but the snapshot was
     * lost). Running it here, inside a transaction, fixes that.
     *
     * <p>De-duplicated against an already-open disconnect alert for the visit —
     * shared with the watcher, so the patient is never double-alerted.
     */
    @Transactional
    public void handleMonitoringDeviceDisconnect(UUID deviceId) {
        DeviceSession session = sessionRepository
                .findByDeviceIdAndSessionActiveTrueAndIsActiveTrue(deviceId)
                .orElse(null);
        if (session == null) return;
        com.smartTriage.smartTriage_server.module.visit.entity.Visit visit = session.getVisit();
        if (visit == null) return;

        boolean alreadyOpen = clinicalAlertRepository
                .existsByVisitIdAndAlertTypeAndIsAcknowledgedFalseAndIsActiveTrue(
                        visit.getId(),
                        com.smartTriage.smartTriage_server.common.enums.AlertType.IOT_DEVICE_DISCONNECTED);
        if (!alreadyOpen) {
            IoTDevice device = session.getDevice();
            // Target the patient's zone + accountable doctor so the alert is user-targeted
            // and visible to zone-scoped queries (not just a hospital-wide broadcast).
            com.smartTriage.smartTriage_server.common.enums.EdZone zone =
                    visit.getCurrentTriageCategory() != null
                            ? com.smartTriage.smartTriage_server.common.enums.EdZone
                                    .fromTriageCategory(visit.getCurrentTriageCategory())
                            : null;
            com.smartTriage.smartTriage_server.module.user.entity.User zoneDoctor =
                    resolveZoneDoctorForDisconnect(visit.getPatient().getHospital().getId(), zone);
            com.smartTriage.smartTriage_server.module.alert.entity.ClinicalAlert alert =
                    com.smartTriage.smartTriage_server.module.alert.entity.ClinicalAlert.builder()
                            .visit(visit)
                            .alertType(com.smartTriage.smartTriage_server.common.enums.AlertType.IOT_DEVICE_DISCONNECTED)
                            .severity(com.smartTriage.smartTriage_server.common.enums.AlertSeverity.HIGH)
                            .title("Monitor offline — patient unmonitored")
                            .message(String.format(
                                    "Monitor '%s' (Serial: %s) for patient %s %s (Visit: %s) has lost "
                                            + "its heartbeat (last seen: %s). The patient is no longer being "
                                            + "continuously monitored. Check the device, power it back on, or "
                                            + "pair a replacement.",
                                    device != null ? device.getDeviceName() : "?",
                                    device != null ? device.getSerialNumber() : "?",
                                    visit.getPatient().getFirstName(),
                                    visit.getPatient().getLastName(),
                                    visit.getVisitNumber(),
                                    device != null ? device.getLastHeartbeatAt() : null))
                            .targetZone(zone)
            ...
            broadcastDisconnectAlert(visit, alert, zone, zoneDoctor);
            log.warn("Monitor {} disconnected during active session for Visit {} — alert raised, "
                            + "state → DISCONNECTED.",
                    device != null ? device.getSerialNumber() : "?", visit.getVisitNumber());
        }

        // Freeze the last live reading before moving to DISCONNECTED so a page
        // reload shows last-known values rather than an older periodic snapshot.
        snapshotLatestStreamForSession(session.getId());
        // Keep the session OPEN — transition to DISCONNECTED so the clinical
        // timeline stays one continuous record when the device reconnects.
        transitionSessionState(session.getId(),
                com.smartTriage.smartTriage_server.common.enums.MonitoringState.DISCONNECTED);
    }

    /**
     * Fan a device-disconnect alert out to the hospital board + the patient's zone + the
     * accountable zone doctor + the charge nurse(s), AFTER COMMIT — so an unmonitored
     * patient is actively paged to the responsible clinicians, not just broadcast to
     * dashboards, and a rolled-back disconnect-handling tx never pushes a phantom alert.
     * Best-effort — a STOMP failure must not abort the disconnect-handling transaction.
     */
    private void broadcastDisconnectAlert(
        ...
        try {
            ...
            com.smartTriage.smartTriage_server.module.alert.dto.ClinicalAlertResponse response =
                    com.smartTriage.smartTriage_server.module.alert.mapper.ClinicalAlertMapper.toResponse(alert);
            java.util.List<UUID> userIds = new java.util.ArrayList<>();
            if (zoneDoctor != null) userIds.add(zoneDoctor.getId());
            for (var cn : shiftAssignmentService.getChargeNurse(hospitalId)) {
                if (cn != null) userIds.add(cn.getId());
            }
            eventPublisher.publishOwnedAlertAfterCommit(hospitalId, zone, response, userIds);
        } catch (Exception e) {
            log.warn("Failed to broadcast device-disconnect alert for visit {}: {}",
                    visit.getVisitNumber(), e.getMessage());
        }
    }

    /** Accountable zone doctor for the patient's current zone, or null if none on shift. */
    private com.smartTriage.smartTriage_server.module.user.entity.User resolveZoneDoctorForDisconnect(
            UUID hospitalId, com.smartTriage.smartTriage_server.common.enums.EdZone zone) {
        try {
            if (hospitalId == null || zone == null) return null;
            var doctors = shiftAssignmentService.getDoctorsForZone(hospitalId, zone);
