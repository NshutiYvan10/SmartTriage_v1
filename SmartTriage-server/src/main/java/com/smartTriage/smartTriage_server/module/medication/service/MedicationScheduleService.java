...
public class MedicationScheduleService {
    ...
    private void raiseAdministrationOverrideAlert(
        ...
        // Notify the floor/safety lead live — an administration-time override is a
        // point-of-harm event. Best-effort; never block the administration tx.
        try {
            var visit = order.getVisit();
            if (visit != null && visit.getHospital() != null) {
                var resp = ClinicalAlertMapper.toResponse(alert);
                realTimeEventPublisher.publishHospitalAlert(visit.getHospital().getId(), resp);
                if (visit.getCurrentEdZone() != null) {
                    realTimeEventPublisher.publishZoneAlert(
                            visit.getHospital().getId(), visit.getCurrentEdZone(), resp);
                }
            }
        } catch (Exception e) {
            log.warn("Failed to publish administration override alert {}: {}", alert.getId(), e.getMessage());
        }
