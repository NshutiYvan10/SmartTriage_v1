...
public class EmsRunService {
    ...
    /**
     * Personal work-loop notification back to the transporting paramedic (Alert
     * Center feed + /topic/alerts/user/{id} push): the ED acted on their run —
     * receipt acknowledged, handover complete, field triage confirmed.
     * {@code targetDoctor = the run's paramedic} is the delivery key:
     * AlertScopeResolver gives PARAMEDIC a personal-only scope (alerts where
     * targetDoctor is them), so these — and ONLY these — appear in their feed,
     * with no hospital/zone clinical noise. Requires the run's visit (the ack
     * endpoint's canAccessAlert walks alert→visit→hospital; by ARRIVED a visit
     * always exists). Best-effort: never blocks the workflow transition.
     */
    private void notifyParamedic(EmsRun run, AlertType type, AlertSeverity severity,
                                 String title, String message) {
        try {
            if (run.getParamedic() == null || run.getVisit() == null || run.getHospital() == null) {
                return;
            }
            ClinicalAlert alert = ClinicalAlert.builder()
                    .visit(run.getVisit())
                    ...
                    .title(title)
                    .message(message)
                    .targetDoctor(run.getParamedic())
                    .autoGenerated(true)
                    .build();
            alert = clinicalAlertRepository.save(alert);
            realTimeEventPublisher.publishOwnedAlertAfterCommit(
                    run.getHospital().getId(), null, ClinicalAlertMapper.toResponse(alert),
                    List.of(run.getParamedic().getId()));
        } catch (Exception e) {
            log.warn("[ems] Paramedic notification failed for run {}: {}", run.getId(), e.getMessage());
        }
    }
    ...
    public EmsRunResponse acknowledgeArrival(UUID runId) {
        ...
        if (run.getArrivalAckedAt() == null) {
            ...
            notifyParamedic(run, AlertType.EMS_ARRIVAL_ACKNOWLEDGED, AlertSeverity.LOW,
                    "ED received your patient",
                    String.format("Receipt of your patient%s was acknowledged at the door by %s.",
                            run.getUnitCallsign() != null && !run.getUnitCallsign().isBlank()
                                    ? " (unit " + run.getUnitCallsign() + ")" : "",
                            safe(run.getArrivalAckedByName(), "ED")));
    ...
    public EmsRunResponse transferOfCare(UUID runId, TransferOfCareRequest req) {
        ...
        // MEDIUM (not LOW): this is the transfer of legal responsibility — the
        // paramedic is released from the run and should positively note it.
        notifyParamedic(run, AlertType.EMS_HANDOVER_COMPLETE, AlertSeverity.MEDIUM,
                "Handover complete — care transferred",
                String.format("Care of your patient was formally accepted by %s. You are released from this run.",
                        safe(run.getHandedOffToName(), "the receiving clinician")));
